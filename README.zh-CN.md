...
TMCRA 是 Agent 系统的长期记忆层。它帮助大语言模型在长对话历史中检索、连接和推理相关记忆，而不需要在每一轮都暴露完整上下文。
...
TMCRA 在 Agent 应用和回答模型之间增加一层专门的长期记忆层。
...
TMCRA 将记忆组织成图节点和学习得到的召回路径，再把压缩后的证据提供给回答模型。目标是让外部 Agent 可以通过长期记忆层/API 接口使用长期记忆，同时保持记忆算法和模型权重可以独立部署。
...
本仓库是 TMCRA 长期记忆层工作的对外展示包，适合用于：

- benchmark 审阅
- 模型和结果检查
- 复现冻结基线
- 展示 TMCRA 如何作为外部 Agent 的长期记忆层进行封装
